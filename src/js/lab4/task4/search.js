...
module.exports = class Search {
  ...
  findSimilar(arr) {
    arr.forEach((element) => {
      if (this.searchExp(element)) {
        ...
        const listItem = this.createItem(element, this.searchFindStr(element));
  ...
  searchFindStr(element) {
    if (element.name.first.search(this.getValue()) !== -1) {
      const str = element.name.first;
      return `${this.insertMark(
        str,
        str.search(this.getValue()),
        this.getValue().length,
      )} ${element.name.last}, ${element.dob.age}, `;
    } if (element.dob.age.toString().search(this.getValue()) !== -1) {
      const str = element.dob.age.toString();
  ...
  listenClick(users, favUsers) {
    const items = this.findList.getAllItems();
    items.forEach((element) => {
      element.addEventListener('click', (event) => {
        this.info_popup.openContainer();
        const cardId = event.target.dataset.card;
        const elem = this.info_popup.getElem(users, cardId);
