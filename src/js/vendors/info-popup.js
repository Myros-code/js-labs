...
module.exports = class infoPopup extends Popup {
  ...
  listen(users) {
    ...
    this.trigers.forEach((el) => {
      el.onclick = (event) => {
        this.openContainer();
        const card = event.target.closest('.popup-trigger');
        const cardId = card.id;
        const elem = this.getElem(users, cardId);
  ...
  getElem(users, cardId) {
    return users.find((el) => el.id.value == cardId);
  }

  toggleFavorite(elem, users) {
    const res = users.findIndex((el) => el.id.value === elem.id.value);
  ...
  getTimetoBday(elem) {
    const nowDate = dayjs();
    const dateB = dayjs(elem.dob.date).set('year', dayjs().year());
    const left = Math.abs(dateB.diff(nowDate, 'days'));
